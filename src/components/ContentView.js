...
export function ContentView({ saveData, updateSaveData, userOrganisms, updateUserOrganisms, userOrganismUpgrades, organisms, shroomShopItems, enviroShopItems }) {
  // choose which view to render based on the selected option
  return (
    <Switch>
      <Route exact path="/play/sector">
        <Sector savedata={saveData} updateSaveData={updateSaveData} userOrganisms={userOrganisms} updateUserOrganisms={updateUserOrganisms} userOrganismUpgrades={userOrganismUpgrades} organisms={organisms}/>
      </Route>
